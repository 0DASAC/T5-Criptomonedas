...
class BlockMessage:
    ...
    def __init__(self, block):
        self.block = block  # FullBlock

    @classmethod
    def parse(cls, stream):
        # El payload de 'block' es un bloque completo
        fb = FullBlock.parse(stream)
        return cls(fb)
...
###############################
    def serialize(self):
        # count (varint) + entradas: type (4 LE) + hash (32 LE)
        result = encode_varint(len(self.data))
        for data_type, identifier in self.data:
            result += int_to_little_endian(data_type, 4)
            # el hash para el wire va en little-endian
            result += identifier[::-1]
        return result
